...
use crate::{
    ...
    }, model::{cell_meta::{ CellMeta, CELL_TYPE_GEOM, CELL_TYPE_LABEL, CELL_TYPE_MULTY }, mx_cell::MxCell}, store::cell::{self, cell_type_compare, CellInfoContext, SetCellTypeAction, StartApplyAction, NOT_CELL, NO_CONTEXT_FOUND }, utils::set_widget_model
};
...
pub fn CellDetails() -> Html {
    ...
    let cell_meta_handler = use_state(|| {
            context.mx_cell.get_meta().unwrap()
        });
